//
//  JKFilePathOperationExtension.m
...
@implementation JKFilePathOperationExtension
...
-(NSArray *)jKDeepSearchSubpathsAtPathAllFileNames:(NSString *)folderName
{
    NSFileManager *fileManager = [NSFileManager defaultManager];
    // 取得一个目录下得所有文件名
    NSArray *files = [fileManager subpathsAtPath:[self jk_createFolder:folderName]];
...
-(NSArray *)jKDeepSearchEnumeratorAllFileNames:(NSString *)folderName{
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    // 取得一个目录下得所有文件名
    NSDirectoryEnumerator *files = [fileManager enumeratorAtPath:[self jk_createFolder:folderName]];
    //NSLog(@"pdf名字的数量=%ld 数组=%@",files.count,files);
...
-(NSString *)jKCalculateTheSizeOfTheFolderPath:(NSString *)folderPath{
    ...
    if (isExist) {
        ...
        if (folderSize >= 1024.0*1024.0*1024.0) {
            ...
            sizeString = [NSString stringWithFormat:@"%.2fG",folderSize / (1024.0*1024.0*1024.0)];
